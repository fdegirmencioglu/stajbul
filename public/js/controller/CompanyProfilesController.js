app.controller('CompanyProfilesController', function ($scope, $upload, companyProfilesFactory, profilesFactory, logsFactory) {
    ...
    companyProfilesFactory.get(groupID).then(function (d) {
    ...
    $scope.delete_company = function (id) {
        companyProfilesFactory.remove(id).then(function (d) {
            var sonuc = d.data;
        });
        window.location.reload();
    }

    //firma bilgilerini Yönetici Olarak Düzenle
    $scope.edit_company = function (edit_company_id) {

        /*companyProfilesFactory.get_current_company().then(function (obj) {
         $scope.aktif_kullanici = obj.data[0];*/
        var temp_company = {};
        $scope.temp_company = companyProfilesFactory.get_user(edit_company_id);

        /* $scope.auth_person = $scope.temp_company.yetkili_adi;
         $scope.the_auth_to = $scope.temp_company.yetkili_pozisyonu;
         $scope.year_of_establishment = $scope.temp_company.kurulus_yili;
         $scope.number_of_employees = $scope.temp_company.calisan_sayisi;
         $scope.display_your_name = $scope.temp_company.display_name;
         $scope.company_about_editor = $scope.temp_company.firma_bilgileri;
         //$scope.correctlySelected.value = $scope.temp_copmpany.;        
         $scope.correctlySelected = $scope.temp_company.sehir;
         $scope.county = $scope.temp_company.ilce;
         $scope.phone_number = $scope.temp_company.telefon;
         $scope.fax_number = $scope.temp_company.fax;
         $scope.address = $scope.temp_company.adress;*/
        /*
         options.aktif_kullanici_id = kullanici_id_;
         options.aktif_firma_id = aktif_kullanici_firma_id;
        ...
        console.log("$scope.temp_company");
        console.log($scope.temp_company);

        /* console.log("$scope.temp_company.yetkili_adi");
         console.log($scope.temp_company);*/
